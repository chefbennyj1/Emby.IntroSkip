...
namespace IntroSkip.Chapters
{
    public class ChapterInsertion : IServerEntryPoint
    {
        ...
        public void InsertIntroChapters(long id, TitleSequenceResult titleSequence)
        {
            ...
            if (config.EnableChapterInsertion)
            {
                ...
                foreach (var chap in getChapters)
                {
                    ...
                    Log.Debug("CHAPTER INSERT: Fetch Existing Chapters: {0} Starts at {1}", chap.Name, ConvertTicksToTime(chap.StartPositionTicks));
                }
        ...
        #region EndCreditChapterCreation

        /*public void InsertEndCreditChapters(long id)
        {
            Log.Debug("CHAPTER END CREDIT INSERT: PASSED ID from TASK = {0}", id);
            ...
            Log.Info("CHAPTER END CREDIT INSERT: TV Show: {0} - {1}", tvShowName, seasonName);
            Log.Info("CHAPTER END CREDIT INSERT: Getting Chapter Info for {0}: {1}", episodeNo, item.Name);
            ...
            if (config.EnableEndCreditChapterInsertion)
            {
                //Lets get the existing chapters and put them in a new list so we can insert the new Intro Chapter
                foreach (var chap in getChapters)
                {
